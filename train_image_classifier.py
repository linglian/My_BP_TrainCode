...
def preprocessing(image, is_color_jitter=False):
    ...
    crop_width = tf.random_uniform([], minval= shape[1] * tf.convert_to_tensor(0.5),
                                             maxval=shape[1], dtype=tf.float32)

    crop_height = tf.to_int32(crop_height)
    crop_width = tf.to_int32(crop_width)
